...
/*
 * Command for toggling debug flags.
 */
static
int
cmd_df(int nargs, char **args)
{
	...
	// check if 2 arguments given, if first arg is an int, and if in range of df
	if (nargs != 3 || atoi(args[1]) < 1 || atoi(args[1]) > 12){
                kprintf("Usage: df nr on/off\n");
                return EINVAL;
        }

	// checks if either on or off condition
	if (strcmp(args[2],"on") == 0) {
		kprintf("Debug flag %s toggled ON\n", args[1]);
		dbflags = dbflags | (1<<(atoi(args[1]) - 1));
		return 0;
	}

	if (strcmp(args[2],"off") == 0) {
		kprintf("Debug flag %s toggled OFF\n", args[1]);
		dbflags = dbflags & ~(1<<(atoi(args[1]) - 1));
                return 0;
        }
	...
	// string is incorrect
	kprintf("Usage: df nr on/off\n");
        return EINVAL; //vfs_chdir(args[1]);
}
